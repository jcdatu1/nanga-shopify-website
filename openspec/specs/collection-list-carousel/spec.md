# collection-list-carousel Specification

## Purpose

Provide a merchant-configurable section that renders a curated list of collections as a horizontally scrolling carousel of circular image tiles with uppercase labels, matching the NANGA design. It reuses the theme's native `<carousel-slider>` engine and shared slider classes (no third-party libraries), with per-block image/label overrides, responsive per-row settings that gate scroll and arrows, a left-aligned heading with optional description and "View All" link, and a theme-editor preset.
