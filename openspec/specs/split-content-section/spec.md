# split-content-section Specification

## Purpose

Provide a reusable image-with-text section (`sections/split-content.liquid`, "C Image with text") presenting a full-bleed `object-cover` image filling one half and a color-scheme text panel filling the other, matching the NANGA design's split content band. It offers full-width and page-width layout options, image left/right placement, and panel content composed from heading, subheading, text, and button blocks — distinct from and leaving untouched the stock `image-with-text` section.
