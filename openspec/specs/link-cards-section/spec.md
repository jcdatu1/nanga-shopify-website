# link-cards-section Specification

## Purpose

Provide a reusable link-card grid section (`sections/c-link-cards.liquid`, "C Link cards"): whole-card navigation tiles in two styles — image-topped (4:3 cover with zoom hover) and boxed (white card with shadow hover) — with the theme's established title/arrow hover language, optional heading, and optional band background. Covers the NANGA design's Knowledge grid and Care & Maintenance band; no JavaScript.
