# featured-article-section Specification

## Purpose

Provide the NANGA featured-article hero (`sections/c-featured-article.liquid` + `assets/c-featured-article.css`) for blog templates, reproducing the design's Journal hero (`context/NANGAv1/src/app/pages/Journal.tsx`): one merchant-selectable article as a whole-card split layout — 4:3 image beside meta / title / excerpt / arrow label — with metafield-driven meta and the theme's card hover language.
