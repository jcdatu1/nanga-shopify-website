...
Provide the NANGA Knowledge page as a section-composed template (`templates/page.knowledge.json`) reproducing the design (`context/NANGAv1/src/app/pages/Knowledge.tsx`): flat page header, image link-card grid, boxed Care & Maintenance band, and flat FAQ list — built entirely from the reusable `c-page-header`, `c-link-cards`, and `c-faq-list` sections with the design copy as editable defaults.
